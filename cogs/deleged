...
class Utility(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.snipes = {}
        self.load_data()
        print("✅ Utility Cog initialized")  # Debug
    ...
    @commands.command(name="useravatar", help="Get a user's avatar in high quality")
    ...
    @commands.command(name="base64", help="Encode or decode Base64. Usage: !base64 encode <text> or !base64 decode <base64>")
    async def base64_cmd(self, ctx, mode: str, *, text: str):
        mode = mode.lower()
        if mode == "encode":
            encoded = base64.b64encode(text.encode()).decode()
            embed = self.create_embed("🔐 Base64 Encode", f"**Original:** `{text}`\n**Encoded:** `{encoded}`", 0x57F287)
        elif mode == "decode":
            try:
                decoded = base64.b64decode(text).decode()
                embed = self.create_embed("🔓 Base64 Decode", f"**Encoded:** `{text}`\n**Decoded:** `{decoded}`", 0x57F287)
            except:
    ...
    @commands.command(name="boosters", help="Show all server boosters")
    async def boosters(self, ctx):
        ...
        if not boosters:
            ...
            embed = self.create_embed("🎁 Server Boosters", booster_list, 0x57F287, footer=f"Total: {len(boosters)} boosters | Level: {ctx.guild.premium_tier}")
        await ctx.send(embed=embed)

    @commands.command(name="chat", help="Simple echo command (bot repeats what you say)")
    async def chat(self, ctx, *, message: str):
        embed = self.create_embed("💬 Chat", message, 0x2b2d31, fields=[("👤 User", ctx.author.mention, True)])
        await ctx.send(embed=embed)

    @commands.command(name="chatgpt", help="Simulated ChatGPT response (API required for real)")
    async def chatgpt(self, ctx, *, prompt: str):
        embed = self.create_embed("🤖 ChatGPT", f"**Prompt:** {prompt}\n\n**Response:** This is a simulated response. Add your OpenAI API key for real responses.", 0x2b2d31, fields=[("👤 User", ctx.author.mention, True)])
        await ctx.send(embed=embed)

    @commands.command(name="clearsnipe", help="Clear snipe data for a channel")
    ...
    @commands.command(name="dump", help="Dump recent messages from a channel (Admin only)")
    ...
    async def dump(self, ctx, channel: discord.TextChannel = None, limit: int = 100):
        ...
        if not messages:
            ...
            embed = self.create_embed(f"📄 Message Dump - #{channel.name}", f"```\n{dump_text}\n```", 0x2b2d31, footer=f"Showing last {len(messages)} messages")
        await ctx.send(embed=embed)

    @commands.command(name="embed", help="Create a custom embed. Usage: !embed \"Title\" Description text")
    @commands.has_permissions(manage_messages=True)
    ...
    @commands.command(name="guildbanner", help="Show server banner")
    ...
    @commands.command(name="guildicon", help="Show server icon")
    ...
    @commands.command(name="guildsplash", help="Show server splash/invite background")
    ...
    @commands.command(name="membercount", help="Show member count for the server")
    ...
    @commands.command(name="remind", help="Set a reminder. Usage: !remind 10m Check Discord or !remind 1h30m Meeting")
    async def remind(self, ctx, duration: str, *, reminder: str):
        ...
        embed = self.create_embed("⏰ Reminder Set", f"I'll remind you in {duration}", 0x57F287, fields=[("📝 Reminder", reminder, False), ("⏱️ Time", f"<t:{int(remind_time.timestamp())}:R>", True)])
        await ctx.send(embed=embed)
        ...
        if reminder_exists:
            embed = self.create_embed("⏰ Reminder", f"**{reminder}**", 0x2b2d31, fields=[("⏱️ Set", f"<t:{int(remind_time.timestamp())}:R>", True)])
            await ctx.send(embed=embed)
            self.data["reminders"][str(ctx.author.id)] = [r for r in reminders if r["message"] != reminder or r["time"] != remind_time.isoformat()]
            self.save_data()

    @commands.command(name="reminders", help="List all your active reminders")
    async def list_reminders(self, ctx):
        ...
        if not reminders:
            ...
            embed = self.create_embed("📋 Your Reminders", "\n".join(reminder_list), 0x2b2d31, footer=f"Total: {len(reminders)} reminders")
        await ctx.send(embed=embed)

    @commands.command(name="screenshot", help="Take a screenshot of a website")
    ...
    @commands.command(name="sav", help="Alias for server icon")
    ...
    @commands.command(name="serverinfo", help="Show detailed server information")
    ...
    @commands.command(name="snipe", help="Show the last deleted message in a channel")
    ...
    @commands.command(name="stealemoji", help="Steal an emoji from another server. Usage: !stealemoji :emoji: custom_name")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(emoji_url) as resp:
                    if resp.status == 200:
                        ...
                        embed = self.create_embed("✅ Emoji Stolen", f"Successfully added {new_emoji} with name `{name}`", 0x57F287)
                        await ctx.send(embed=embed)
                    else:
    ...
    @commands.command(name="userbanner", help="Alias for banner command")
    async def user_banner_cmd2(self, ctx, member: discord.Member = None):
        await self.user_banner_cmd(ctx, member)

    @commands.command(name="userinfo", help="Show detailed user information")
    ...
    @commands.command(name="vc", help="Show all voice channels and their member counts")
...
async def setup(bot):
    ...
    print("✅ Utility cog setup complete")
